         ...
         {data.map((log: any, key: number) => (
            <div key={key} className='w-full bg-white shadow px-4 py-2 text-[12px] my-1 rounded-md'>
               ...
               <div className='gap-2 flex flex-shrink py-1'>
                  ...
                  <a target='_blank' rel="noopener noreferrer" href={`https://sepolia.basescan.org/tx/${log.transactionHash}`}className='overflow-hidden text-green-600 underline flex text-ellipsis line-clamp-1'>{log.transactionHash}</a>
               </div>
            </div>
         )
         )}
