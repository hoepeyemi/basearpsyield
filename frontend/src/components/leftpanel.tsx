...
const Leftpanel = () => {
   ...
   return (
      <div className={`w-[20rem] xl:w-[25%] sm:w-[25rem] h-full p-5 flex flex-col flex-none max-lg:bg-white max-lg:z-[40] max-lg:absolute max-lg:top-0 max-lg:shadow-md
         ...
         <div className='card w-full pb-0 h-[20rem] bg-[#4CAF50] flex justify-center items-center text-white text-4xl font-semibold'>
         ...
         <div className='w-full h-full max-h-full bg-[#14213d]/80 mt-4 pt-4 rounded-xl px-3 shrink overflow-hidden'>
            <div className="w-full h-[3.3rem] rounded-full flex justify-center items-center bg-black p-[6px]">
               <button type="button" className={`w-1/2 text-[14px] h-full ${!tab ? 'bg-green-700' : 'bg-transparent'} rounded-3xl`} onClick={() => setTab(false)}>Properties</button>
               <button type="button" className={`w-1/2 text-[14px] h-full ${tab ? 'bg-green-700' : 'bg-transparent'} rounded-3xl`} onClick={() => setTab(true)}>History</button>
            </div>
